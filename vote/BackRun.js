...
http.createServer((req, res)=> {
    ...
    if (pathname === '/getMatchList') {
        ...
        //=>筛选出所有参加比赛的
        //=>如果用户传递了搜索记录，还需要在次基础上把与搜索信息匹配的获取到(不能是自己)
        userAry = userAry.filter((item)=> {
            if (search.length > 0) {
                return item['name'].indexOf(search) > -1 && parseFloat(item['isMatch']) === 1 && item['id'] !== userId;
            }
            return parseFloat(item['isMatch']) === 1 && item['id'] !== userId;
        });
